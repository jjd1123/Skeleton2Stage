...
from MDM.data_loaders.humanml.utils.word_vectorizer import WordVectorizer
from MDM.data_loaders.humanml.utils.get_opt import get_opt
...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        for name in tqdm(id_list):
            try:
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
                if (len(motion)) < min_motion_len or (len(motion) >= 200) or np.any(np.isnan(motion)):
                    continue
                text_data = []
                flag = False
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        abs_base_path = f'./embodied_pose/MDM'
        ...
        opt.meta_dir = './embodied_pose/MDM/dataset'
        self.opt = opt
