...
class HumanoidSMPLIM(HumanoidSMPL):
    ...
    def _setup_character_props(self, key_bodies):
        ...
        if self.residual_force_scale > 0:
            self._num_actions += 6
            # self._num_actions += num_key_bodies*3
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        print(f"Finished generating {len(unique_motion_bodies)} humanoids in {t_finish_gen - t_start:.3f}s!")
...
def compute_humanoid_reward(body_pos, body_rot, target_pos, target_rot, dof_pos, dof_vel, target_dof_pos, target_dof_vel, body_vel, body_ang_vel, dof_obs_size, dof_offsets, body_pos_weights, reward_specs,actions):
    ...
    sub_rewards = torch.stack([dof_reward, vel_reward, body_pos_reward, body_rot_reward,action_reward], dim=-1)
    sub_rewards_names = 'dof_reward,vel_reward,body_pos_reward,body_rot_reward,residual_reward'
