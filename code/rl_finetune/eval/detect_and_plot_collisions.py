...
#     mesh_fns = glob.glob(f"{mesh_fn}/**/*.obj",recursive=True)
#     # mesh_fns = list(filter(lambda x: int(os.path.basename(x).split(".")[0].split("_")[1])<660 and int(os.path.basename(x).split(".")[0].split("_")[1])>=60,mesh_fns))
...
#         all_collisions += collisions.shape[0]
#         total.append(collisions.shape[0] / float(triangles.shape[1]) * 100)
#     with open(f"{args.mesh_fn}/metrics.txt","a") as f:
#         f.write(f"Body Pen: {all_collisions/(len(mesh_fns))}\n")
...
if __name__ == "__main__":
    ...
    max_collisions = args.max_collisions
    mesh_fns = glob.glob(f"{mesh_fn}/**/*.obj",recursive=True)
    # mesh_fns = list(filter(lambda x: int(os.path.basename(x).split(".")[0].split("_")[1])<600 and int(os.path.basename(x).split(".")[0].split("_")[1])>=0,mesh_fns))
    # import ipdb; ipdb.set_trace()
    total = []
    all_collisions = 0
    pabr = tqdm(mesh_fns)
    for mesh_fn in pabr:
        ...
        all_collisions += collisions.shape[0]
        if len(total)%1000==0:
            pabr.set_description(f"Body Pen: {all_collisions/(len(total)+1)}")
        total.append(collisions.shape[0] / float(triangles.shape[1]) * 100)
    with open(f"{args.mesh_fn}/metrics.txt","a") as f:
        f.write(f"Body Pen: {all_collisions/(len(mesh_fns))}\n")
    print(sum(total)/len(total))
    print(all_collisions/(len(mesh_fns)))
        # print(collisions.shape)
