...
filter_list = set(fileToList("../data/splits/ignore_list.txt"))
train_list = set(fileToList("../data/splits/crossmodal_train.txt"))
test_list = set(fileToList("../data/splits/crossmodal_test.txt")).union(set(fileToList("../data/splits/crossmodal_val.txt")))


def split_data(dataset_path):
    # train - test split
    Path(f"train_rl_aist/wavs").mkdir(parents=True, exist_ok=True)
    for wav in glob.glob(f"{dataset_path}/*.wav"):
            split_name = "train_rl_aist"
            sequence = wav.split("/")[-1].split(".")[0]         
