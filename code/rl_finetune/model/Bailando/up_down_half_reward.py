...
def isaac_imitation(pose,player):
    reward = player.phys_proj_bailando(pose)
    return reward
...
class UpDownReward(nn.Module):
    ...
    def forward(self, pose, music, ds_rate,player):
        with torch.no_grad():
        # up: 0, 13, 14 normal vector, which should be 15-12, pose(t=0) = [0,0,0]
            ...
            if use_issac:
                start = time.time()
                isaac_reward = isaac_imitation(pose,player)
            ...
            # reward = 
            print("dir_reward:",reward.mean())
            reward *= self.mrate
            ba_r = ba_reward(pose, music)
            print("ba_reward:",ba_r.mean())
            reward += ba_r
            if use_issac:
                isaac_reward -= 0.5
                isaac_reward *= 10
                print("isaac_reward:", isaac_reward.mean())
                reward += isaac_reward

                return reward.clone().detach(), ba_r, isaac_reward
        return reward.clone().detach(), ba_r, None
