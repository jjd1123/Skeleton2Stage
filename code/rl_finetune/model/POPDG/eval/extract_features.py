...
sys.path.append("/data/PhysDanceRL/code/rl_finetune/model/POPDG")
import numpy as np
...
folder_path = '/data/PhysDanceRL/code/rl_finetune/runs/origin'
save_dir = '/data/PhysDanceRL/code/rl_finetune/runs/origin/features'
...
for filename in tqdm(os.listdir(folder_path)):
    if filename.endswith('.pkl'):  
        ...
        # #     scale = data["scale"][0]
        # # except:
        # #     pos, q = data["smpl_trans"], data["smpl_poses"]
        # #     scale = data["smpl_scaling"][0]
        # # pos /= scale
        # pos = data["transl"]
        # q = data["pred_thetas"]
        # pos = torch.from_numpy(pos).unsqueeze(0)[:,::2]
        
        # q = torch.from_numpy(q).unsqueeze(0).view(1,-1,24,3)[:,::2]
        ...
        # keypoints3d = smpl.forward(q, pos).detach().cpu().numpy().squeeze()#[:,::2]  # b, s, 24, 3
        keypoints3d = data["full_pose"][:600]
        print(keypoints3d.shape)
        root = keypoints3d[...,[0],:]
        # # breakpoint()
        keypoints3d = keypoints3d - root
