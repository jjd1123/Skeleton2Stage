...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            ...
            betas * np.sqrt(alphas_cumprod_prev) / (1.0 - alphas_cumprod), #x0系数
        )
        self.register_buffer(
            "posterior_mean_coef2",
            (1.0 - alphas_cumprod_prev) * np.sqrt(alphas) / (1.0 - alphas_cumprod), #xt系数
        )
    ...
    def p_mean_variance(self, x, cond, t):
        ...
        assert torch.isfinite(max_log).all(), "max_log contains NaN or Inf!"
        
        frac = torch.clamp((model_var_values + 1) / 2, min=0, max=1)
        # assert torch.isfinite(frac).all(), "frac contains NaN or Inf!"
        model_log_variance = frac * max_log + (1 - frac) * min_log
        model_log_variance = torch.clamp(model_log_variance, min=-10, max=10)

        model_variance = torch.exp(model_log_variance) + 1e-9
    ...
    def _vb_terms_bpd(self, x_start, x_t, cond, t):
        ...
        assert torch.isfinite(kl).all(), "KL contains NaN or Inf!"
        ...
        assert torch.isfinite(decoder_nll).all(), "decoder_nll contains NaN or Inf!"
    ...
    def p_losses(self, x_start, cond, t):
        ...
        model_out, model_contact = torch.split(
            model_out, (147, model_out.shape[2] - 147), dim=2
        )
        target, target_contact = torch.split(target, (147, target.shape[2] - 147), dim=2)
        # velocity loss
        ...
        # body skate loss
        body_idx = [7, 8, 10, 11, 15, 20, 21, 22, 23]

        static_idx = model_contact > 0.95  # N x S x 9
        model_body = model_xp[:, :, body_idx]  # body positions (N, S, 9, 3)
        model_body_v = torch.zeros_like(model_body)
        model_body_v[:, :-1] = (
            model_body[:, 1:, :, :] - model_body[:, :-1, :, :]
        )  # (N, S-1, 9, 3)
        model_body_v[~static_idx] = 0
        body_loss = self.loss_fn(
            model_body_v, torch.zeros_like(model_body_v), reduction="none"
        )
