...
def slice_and_clean(motion_dir, wav_dir, stride=0.5, length=5):
    ...
    motion_dir= Path(motion_dir)#, Path(wav_dir)
    motion_out = motion_dir.with_name(f"{motion_dir.name}_sliced")#, wav_dir.with_name(f"{wav_dir.name}_sliced")
    ...
    for motion_file in tqdm(motion_files, total=len(motion_files)):
        # assert motion_file.stem == wav_file.stem, "Mismatched file names."
        # audio_slices = slice_audio(wav_file, stride, length, wav_out)
        motion_slices = slice_motion(motion_file, stride, length, motion_out)
