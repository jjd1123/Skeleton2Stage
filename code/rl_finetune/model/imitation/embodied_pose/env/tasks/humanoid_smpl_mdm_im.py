...
import torch
from embodied_pose.env.tasks.humanoid_smpl_im import HumanoidSMPL, dof_to_obs
from embodied_pose.env.tasks.humanoid_smpl_im_vis import HumanoidSMPLIMVis
...
from embodied_pose.utils.config import flags
...
class Childprocess(Exception):
    pass

TMP_SMPL_DIR = f"/tmp/smpl_humanoid_{uuid4()}"

def _create_smpl_humanoid_xml_new(humanoid_bodies,offset, queue, smpl_robot, pid):
    res = dict()
    print("\rStart running {}...".format(pid),end='')
    for idx,bodies in enumerate(humanoid_bodies):
        model_xml_path = f"{TMP_SMPL_DIR}/smpl_humanoid_{offset+idx}.xml"
        gender_beta = bodies
        rest_joints = smpl_robot.load_from_skeleton(betas=gender_beta[None, 1:], gender=gender_beta[:1], scale= 1.0, model_xml_path=model_xml_path)
        smpl_robot.write_xml(model_xml_path)
        res[",".join([f"{x:.6f}" for x in gender_beta])] = ((offset+idx, model_xml_path, rest_joints))
    print("\rEnd running {}!!!".format(pid),end='')
...
# def wrap_create_smpl_humanoid_xml(humanoid_ids, queue, smpl_robot, motion_bodies, body_scales, pid):
#     return ThreadPoolExecutor().submit(_create_smpl_humanoid_xml,*(humanoid_ids, queue, smpl_robot, motion_bodies, body_scales, pid)).result()
    

class HumanoidSMPLMDMIM(HumanoidSMPLIMVis):
    ...
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        self._state_init = HumanoidSMPLMDMIM.StateInit[state_init]
        # self.mdm = None
        # if cfg["model_path"]!=None:
        #     self.mdm = MDMTalker()
        return
    ...
    def pre_physics_step(self, actions):
        ...
        # dof_actions[:,self._num_dof-6:self._num_dof] = 0
        # dof_actions[:, 12:15] = 0
        # dof_actions[:, 24:27] = 0
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self._reset_ref_motion_ids = torch.arange(self.num_envs, device=self.device)
        self._reset_ref_motion_bodies = torch.zeros((self.num_envs,11),device=self.device)
        all_motion_bodies = self._reset_ref_motion_bodies
        ...
        unique_motion_bodies = np.unique(self._reset_ref_motion_bodies.cpu().numpy(),axis=0)
        ...
        if self.multi_process:
            ...
            for i in range(1, len(jobs)):
                try:
                    worker_args = (job_args[i],queue, smpl_robot,all_motion_bodies,all_motion_body_scales , i)
                    worker = mp.Process(target=_create_smpl_humanoid_xml, args=worker_args)
                    worker.start()
                    workers.append(worker)
                except Childprocess as e:
                    print(str(e))
            # for i in range(1,len(jobs)):
            #     worker = workers.pop()
            #     worker.join()
            for i in tqdm(range(len(jobs) - 1)):
                res = queue.get()
                res_acc += res
            res_acc += (_create_smpl_humanoid_xml(jobs[0],None, smpl_robot, all_motion_bodies,all_motion_body_scales , 0))
            
        else:
            res_acc = dict()
            offset = 0
            for _, idx in enumerate(tqdm(torch.tensor(unique_motion_bodies))):
                res_acc.update(_create_smpl_humanoid_xml_new([idx],offset, None, smpl_robot, 0))
                offset+= 1
            motion_2_id = []
            for i,motion_id in enumerate(motion_ids):
                body = ",".join([f"{x:.6f}" for x in all_motion_bodies[motion_id]])
                motion_2_id.append(res_acc[body][0])
            motion_ids = motion_2_id
            res_acc = res_acc.values()
    ...
    def post_physics_step(self):
        ...
        self._set_target_motion_state() #change

        self._refresh_sim_tensors() #get observation
        ...
        # self.extras["root_rewards"] = self.root_reward
    ...
    def _reset_actors(self, env_ids):
        if (self._state_init == HumanoidSMPLMDMIM.StateInit.Default):
            self._reset_default(env_ids)
        elif (self._state_init == HumanoidSMPLMDMIM.StateInit.Start
              or self._state_init == HumanoidSMPLMDMIM.StateInit.Random):
            self._reset_ref_state_init(env_ids)
        elif (self._state_init == HumanoidSMPLMDMIM.StateInit.Hybrid):
    ...
    def _reset_ref_state_init(self, env_ids):
        ...
        self._resample_amass_motions()
        motion_ids = self._reset_ref_motion_ids
        if use_env_ids:
            motion_ids = motion_ids[env_ids]

        if (self._state_init == HumanoidSMPLMDMIM.StateInit.Random
            or self._state_init == HumanoidSMPLMDMIM.StateInit.Hybrid):
    ...
    def _compute_reward(self, actions):
        ...
        cfg_reward_specs = self.cfg['env'].get('reward_specs', dict())  #0.6,0.1,0.2,0.1 1171
        ...
        if torch.any(reset_mask):
            ...
            # self.root_reward[reset_mask] = 0
        # print(self.root_reward)
        return
    ...
    def get_mdm_state(self,env_ids):
        ...
        pose_quat = torch.from_numpy(sRot.from_rotvec(dof_pos.reshape(-1, 3).cpu().numpy()).as_quat().reshape(23, 4)).to(self.device)
        pose_quat = torch.concatenate([root_rot[None,:],pose_quat],dim=0)
...
def compute_humanoid_reward(body_pos, body_rot, target_pos, target_rot, dof_pos, dof_vel, target_dof_pos, target_dof_vel, body_vel, body_ang_vel, dof_obs_size, dof_offsets, body_pos_weights, reward_specs):
    ...
    # root_reward = target_pos[:,0,2] - body_pos[:,0,2]
    # root_reward = (root_reward ** 2)
    # root_reward = torch.exp(-100 * root_reward)
    
    # body rot reward
    diff_body_rot = quat_mul(target_rot, quat_conjugate(body_rot))
    # print("body_rotation:", body_rot.shape)
    ...
    return reward, sub_rewards, sub_rewards_names#,root_reward
