...
import torch
from embodied_pose.env.tasks.humanoid_smpl import HumanoidSMPL, dof_to_obs
...
class HumanoidSMPLIM(HumanoidSMPL):
    ...
    def pre_epoch(self, epoch,train_epoch):
        # TODO: resample without destroying the sim
        # if train_epoch == 1:
        #     self._load_motion(f"{ROOT_PATH}/data/tmp")
        #     self._resample_edge_motions()
        if (epoch > 1) and epoch % self.resample_interval == 1:
    ...
    def _resample_edge_motions(self):
        self._setup_tensors()
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        # self._reset_ref_motion_ids = torch.arange(self.num_envs, device=self.device) % 2048
        ...
        # self._reset_ref_motion_bodies = torch.zeros((self._motion_lib.,11),device=self.device)
        ...
        # motion_ids = self._reset_ref_motion_ids.cpu().numpy()
        # unique_motion_bodies = np.unique(self._reset_ref_motion_bodies.cpu().numpy(),axis=0)
        # print('sampled motion bodies:', "{}|{}".format(unique_motion_bodies.shape,
        #                                                np.unique(all_motion_bodies.numpy(),axis=0).shape))
    ...
    def _load_motion(self, motion_file):
        ...
        # self._motion_lib = self._motion_lib.to(device)
    ...
    def _compute_reward(self, actions):
        ...
        self.rew_buf[:], self._sub_rewards, self._sub_rewards_names = compute_humanoid_reward(body_pos, body_rot, target_pos, target_rot, dof_pos, dof_vel, 
                                                                                              target_dof_pos, target_dof_vel, body_vel, body_ang_vel, self._dof_obs_size, self._dof_offsets, self.body_pos_weights, reward_specs,actions)
...
@torch.jit.script
def compute_humanoid_reward(body_pos, body_rot, target_pos, target_rot, dof_pos, dof_vel, target_dof_pos, target_dof_vel, body_vel, body_ang_vel, dof_obs_size, dof_offsets, body_pos_weights, reward_specs,actions):
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, int, List[int], Tensor, Dict[str, float],Tensor) -> Tuple[Tensor, Tensor, str]
    ...
    action_reward = (actions[:,69:]**2).mean(dim=-1)
    action_reward = torch.exp(-30 * action_reward)
    ...
    reward = w_dof * dof_reward + w_vel * vel_reward + w_pos * body_pos_reward + w_rot * body_rot_reward #+ 0.1*action_reward
