...
class MotionLib():
    ...
    def _compute_motion_dof_vels(self, motion):
        ...
        local_rot0 = motion.local_rotation[:-1]
        local_rot1 = motion.local_rotation[1:]
        frame_dof_vel = self._local_rotation_to_dof_vel_batch(local_rot0,local_rot1,dt)
        dof_vels = torch.concatenate([frame_dof_vel,frame_dof_vel[[-1]]],dim=0)
        # for f in range(num_frames - 1):
        #     local_rot0 = motion.local_rotation[f]
        #     local_rot1 = motion.local_rotation[f + 1]
        #     frame_dof_vel_origin = self._local_rotation_to_dof_vel(local_rot0, local_rot1, dt)
        #     frame_dof_vel_origin = frame_dof_vel_origin
        #     dof_vels_origin.append(frame_dof_vel_origin)
        
        # dof_vels_origin.append(dof_vels_origin[-1])
        # dof_vels_origin = torch.stack(dof_vels_origin, dim=0)
        # print(f"total sum:{torch.sum((dof_vels_origin-dof_vels)**2)}")
        # print(f"{dof_vels.shape},{dof_vels_origin.shape}")
        return dof_vels
    
    def _local_rotation_to_dof_vel_batch(self, local_rot0, local_rot1, dt):
        body_ids = self._dof_body_ids
        dof_offsets = self._dof_offsets

        n=local_rot0.shape[0]
        dof_vel = torch.zeros((n,self._num_dof), device=self._device)
