...
from embodied_pose.utils.config import set_np_formatting, get_args, parse_sim_params, load_cfg,get_train_args
...
def get_agent(device=None):
    ...
    set_np_formatting()
    args = get_train_args(device=device)
    cfg, cfg_train = load_cfg(args)
    ...
    if runner.load_check_point and (runner.load_path is not None):
        print(runner.load_path)
        agent.restore(runner.load_path)
    del runner
    del algo_observer
    return agent
