...
import embodied_pose.learning.common_agent as common_agent
import embodied_pose.learning.amp_datasets as amp_datasets
from embodied_pose.utils.tools import AverageMeter, get_eta_str
...
class ImitatorAgent(common_agent.CommonAgent):
    ...
    def train(self,epoch):
        ...
        while True:
            if hasattr(self.task, 'pre_epoch'):
                self.train_schedule += 1
                self.task.pre_epoch(self.epoch_num,self.train_schedule)
            ...
            if self.accelerator.is_main_process:
                if self.rank == 0:
                    ...
                    self.writer.add_scalar('losses/a_loss', self.log_dict['losses/a_loss'], epoch_num)
                    self.writer.add_scalar('losses/c_loss', self.log_dict['losses/c_loss'], epoch_num)
                    self.writer.add_scalar('reward',self.step_rewards.avg.item(),epoch_num)
                    ...
                    # save models
                    latest_model_file = "/mnt/Nerf001/jidong/EDGE/model/imitation/results/amass_im/models/Humanoid_latest"
                    file = os.path.join(self.network_path, f"{self.config['name']}_latestnew")
                    if self.save_freq > 0:
    ...
    def _eval_critic(self, obs_dict):
        ...
        value = self.model.module.a2c_network.eval_critic(obs_dict)
