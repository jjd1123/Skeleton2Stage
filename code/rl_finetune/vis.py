...
def skeleton_render(
    ...
    if render:
        ...
        scat = [
            ax.scatter([], [], [], c=[],zorder=10, s=0, cmap=ListedColormap(["r", "g", "b"]))
            for _ in range(4)
        ]
        axrange = 3
    ...
    if sound:
        ...
        if stitch:
            ...
            audioname = f"{temp_dir.name}/tempsound.wav" if render else os.path.join(out, f'{i}_{epoch}_{"_".join(os.path.splitext(os.path.basename(name[0]))[0].split("_")[:-1])}.wav')
            sf.write(audioname, total_wav, sr)
            outname = os.path.join(
                out,
                f'{i}_{epoch}_{"_".join(os.path.splitext(os.path.basename(name[0]))[0].split("_")[:-1])}.mp4',
            )
        else:
